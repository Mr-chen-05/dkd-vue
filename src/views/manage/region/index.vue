      ...
      >
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['manage:region:query']"
          >
    ...
    >
      <div class="region-detail-container">
        ...
        <div class="region-name-section">
          <el-form :model="{}" label-width="75px">
            <el-form-item label="区域名称:">
              <el-input v-model="regionalName" disabled />
            </el-form-item>
          </el-form>
        </div>

        <!-- 包含点位部分 -->
        <div class="nodes-section" style="margin-top: 30px;">
          <div class="section-title" style="margin-bottom: 15px; font-weight: bold;">包含点位：</div>
          
          <el-table :data="regionDetail" border>
            <el-table-column label="序号" type="index" width="80" align="center" />
            <el-table-column label="点位名称" align="center" prop="nodeName" min-width="200" />
            <el-table-column label="设备数量" align="center" prop="vmCount" width="120" />
          </el-table>
        </div>
      </div>
    </el-dialog>
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除区域为"' + _regionNames + '"的数据项？')
    .then(function () {
...
const handleSelect = (row) => {
  regionalName.value = row.regionName;
  // 查询点位列表
  loadAllParam.regionId = row.id;
  listNode(loadAllParam).then((response) => {
    regionDetail.value = response.rows;
  });
  regionDia.value = true;
};
